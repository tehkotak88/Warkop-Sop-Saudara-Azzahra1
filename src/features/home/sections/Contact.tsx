...
const Contact = () => {
  ...
  const whatsappUrl = `https://wa.me/${selectedOutlet.whatsappNumber}?text=${encodeURIComponent(
  ...
  const contactInfo = [
    ...
    },
    {
      ...
      lines: [`${selectedOutlet.name}: ${selectedOutlet.phone}`],
    },
  ];
      ...
      <div className="container relative z-10 mx-auto px-6 lg:px-12">
        ...
        <div className="mb-16 text-center animate-blur-in">
          ...
          <h2 className="font-serif text-4xl font-bold text-white md:text-6xl lg:text-7xl">
            Lokasi{' '}
            <span className="text-white/30 italic">Cabang</span>
          </h2>
        </div>

        {/* Branch Selector */}
        <div className="mb-12 flex flex-wrap justify-center gap-3">
          {SITE_CONFIG.outlets.map((outlet, index) => (
            <button
              key={index}
              onClick={() => setSelectedOutlet(outlet)}
              className={`rounded-full px-6 py-2 text-xs font-bold tracking-wider transition-all duration-300 uppercase ${
                selectedOutlet.name === outlet.name
                  ? 'bg-yellow-500 text-red-950 shadow-[0_0_20px_rgba(234,179,8,0.3)]'
                  : 'bg-stone-900 text-white/50 border border-stone-800 hover:border-yellow-500/30 hover:text-white'
              }`}
            >
              {outlet.name}
            </button>
        ...
        <div className="grid gap-12 lg:grid-cols-2">
          ...
          <div className="space-y-6">
            <div className="mb-4">
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-2xl font-bold text-white">{selectedOutlet.name}</h3>
                <div className="flex items-center gap-2 rounded-lg bg-white/5 px-3 py-1.5 border border-white/10">
                  <span className="text-yellow-400 text-lg">★</span>
                  <span className="text-white font-bold">{selectedOutlet.rating}</span>
                  <span className="text-white/40 text-xs">({selectedOutlet.reviewCount})</span>
                </div>
              </div>
              <div className="h-1 w-20 bg-yellow-500 rounded-full mb-6" />
              
              {/* Testimonial snippet style from image */}
              <div className="relative rounded-xl bg-white/5 p-4 border-l-4 border-yellow-500/50 mb-8 italic text-white/70 text-sm">
                <span className="absolute -top-3 -left-2 text-4xl text-yellow-500/20 font-serif">"</span>
                {selectedOutlet.testimonial}
                <span className="absolute -bottom-6 -right-2 text-4xl text-yellow-500/20 font-serif">"</span>
              </div>
            </div>
          ...
          {/* Right - Map */}
          <div className="overflow-hidden rounded-2xl border border-stone-800 shadow-2xl shadow-red-950/10 min-h-[450px]">
            <iframe
              key={selectedOutlet.mapsEmbedUrl}
              src={selectedOutlet.mapsEmbedUrl}
              width="100%"
              height="100%"
              ...
              title={`Lokasi ${selectedOutlet.name}`}
            />
          </div>
        </div>
      </div>
